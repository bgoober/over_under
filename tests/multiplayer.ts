...
describe("over_under", () => {
  ...
  const [global] = web3.PublicKey.findProgramAddressSync(
    [Buffer.from("global"), player1.publicKey.toBuffer()],
    program.programId
  );
  ...
  it("Bet placed!", async () => {
    ...
    const [bet] = web3.PublicKey.findProgramAddressSync(
      [Buffer.from("bet"), round.toBuffer(), player1.publicKey.toBuffer()],
      program.programId
    );
    ...
    const [bet2] = web3.PublicKey.findProgramAddressSync(
      [Buffer.from("bet"), round.toBuffer(), player2.publicKey.toBuffer()],
      program.programId
    );
    ...
    const tx2 = await program.methods
      .placeBet(amount2BN, 0, roundNumber2BN) // Use BN objects for the first and third arguments
      .accounts({
        house: player1.publicKey,
        global,
        round,
